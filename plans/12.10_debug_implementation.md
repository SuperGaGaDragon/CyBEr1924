...
#### Step 1.1: 打开后端日志抓取真相
- **文件**: `api.py:109-128`
- [] **目标**: 让 `get_current_user` 明确记录当前请求是“没带 header”还是“decode 失败”，避免盲修。
- **实施步骤**:
  1. 输出 `credentials is None` 或 token 前缀（避免打印完整 token，打 `token[:20]`）；
  2. 输出 `decode_access_token` 是否返回了 user_id，并把 user_id/logged-in state 记录到标准输出；
  3. 如果 decode 成功但 `get_user_by_id` 失败，额外打印 user_id。
- **验证**:
