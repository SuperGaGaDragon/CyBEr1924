...
Phase 3. **恢复 Planner 章节派遣**
   - 3.1 `run_flow.py` 的 novel-mode循环分叉示意：
      Coordinator 轮询 `get_next_pending_subtask()`；如果仍有 `PENDING` subtask，按常规推进；
      若返回 `None`，说明当前 batch 完成。此时必须检查 `state.chapter_candidates`（planner 最近一次写入 `artifact/planner_outline.json`）以及 `state.novel_mode`；
       如果 `state.chapter_candidates` 内含 `PENDING`/`ACCEPT` 章节，立即把它们 append 到 `state.subtasks`（每个附带 `subtask_id`、`type='chapter'`），并重新 `continue` 循环以调度；
      只有当 `chapter_candidates` 为空或都为 `DONE` 才可 `break`。
   -3.2  `Planner` 通常在 `planner.expand_chapters()` 里写入 artifact（`sessions/.../artifacts/{hash}.md`），然后通过 `save_orchestrator_state` 记录 `chapter_candidates`；需要在 log 里增加 `[Planner] Writing chapter candidates: ...`，然后在 coordinator 发现 `state.chapter_candidates` 时把它们 copy 到 `state.subtasks` 并落盘，避免 artifact 只是存文件未入队。
