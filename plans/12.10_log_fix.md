...
Phase 7. **修复章节验证逻辑的双重过滤问题（P0 关键修复）** ⚠️

**问题发现**（代码审查 12.11）：
虽然 Phase 6 在 `_append_chapter_tasks_from_planner_stub` 中做了宽松验证（接受"章"等中文），但章节在传递给 `_apply_planner_result_to_state` 后，**会被第二次严格验证拒绝掉**！

**根本原因分析**：

1. **双重验证冲突**
   - **第一次验证**（Phase 6，行 1308-1327）：宽松验证，接受 "chapter" / "章" / "Chapter"
   - **第二次验证**（行 918-922）：严格验证，要求标题包含 "Chapter {n}" **且** 描述包含 `CHAPTER_FULL_CONTENT`

2. **过滤流程**
   ```
   Phase 6 宽松验证 → 生成 sanitized_subtasks (包含中文章节)
                     ↓
   传递给 _apply_planner_result_to_state
                     ↓
   行 918: _is_valid_chapter_candidate 严格验证
                     ↓
   ❌ 全部被过滤掉（因为缺少 CHAPTER_FULL_CONTENT）
                     ↓
   updated_plan.subtasks 只有 t1-t4
                     ↓
   行 1369-1376: added = 0 (没有新章节)
   ```

3. **日志表象**
   ```
   [Planner] After sanitization: 5 valid chapters  ← Phase 6 通过了
   [Planner] No chapter tasks were added           ← 被 _apply_planner_result_to_state 拒绝
   ```
...
### **方案 A：在 Phase 6 构造时添加 CHAPTER_FULL_CONTENT**（推荐）

**修改位置**：`run_flow.py` 第 1317 行
...
# 修改后：确保总是调用 _chapter_description 来添加 CHAPTER_FULL_CONTENT
desc = _chapter_description(sub.description or title)
```

**原理**：
- `_chapter_description` 函数会自动在描述末尾添加 `CHAPTER_FULL_CONTENT`
- 这样 Phase 6 生成的章节就能通过 `_apply_planner_result_to_state` 的严格验证

**优点**：
- ✅ 最小改动（只改一行）
- ✅ 利用现有的 `_chapter_description` 函数
- ✅ 确保所有章节任务都有统一的描述格式

**缺点**：
- ⚠️ 如果 planner 返回的章节已经有详细描述，会被 `CHAPTER_FULL_CONTENT` 覆盖

---

### **方案 B：放宽 _is_valid_chapter_candidate 的验证条件**

**修改位置**：`run_flow.py` 第 87-90 行
...
def _is_valid_chapter_candidate(raw: Dict[str, Any]) -> bool:
    title = raw.get("title") or ""
    # 只验证标题是否包含章节标识，不再强制要求描述包含 CHAPTER_FULL_CONTENT
    return _chapter_title_has_index(title)
```

**原理**：
- 移除对 `CHAPTER_FULL_CONTENT` 的强制要求
- 只要标题包含 "Chapter N" 或 "第X章"，就接受
...
**缺点**：
- ⚠️ 可能接受一些不完整的章节任务
- ⚠️ 需要修改 `_normalize_chapter_subtask` 来补充描述

---

### **方案 C：统一验证逻辑（彻底解决）**

**修改位置**：`run_flow.py` 第 1308-1327 行 + 第 918-922 行

**修改思路**：
1. **删除 Phase 6 中的宽松验证**（1308-1316 行）
2. **在 `_append_chapter_tasks_from_planner_stub` 中直接调用 `_normalize_chapter_subtask`**
3. **让 `_apply_planner_result_to_state` 的严格验证成为唯一验证点**

**原理**：
- 使用 `_normalize_chapter_subtask` 来标准化章节格式
- 统一通过 `_is_valid_chapter_candidate` 验证
- 避免双重验证冲突
...
**实施步骤**：
1. 修改 `run_flow.py` 第 1317 行，确保调用 `_chapter_description`
2. 添加日志验证章节描述是否包含 `CHAPTER_FULL_CONTENT`
3. 本地测试验证 `added > 0`

**后续优化**（可选）：
- 如果方案 A 仍有问题，考虑方案 B 或 C
- 重构验证逻辑，建立统一的章节标准化流程
...
**检查点 1**：确认 Phase 6 生成的章节包含 `CHAPTER_FULL_CONTENT`
```python
# 在 1327 行后添加日志
...
**检查点 2**：确认 `_apply_planner_result_to_state` 不会过滤章节
```python
# 在 918 行前添加日志
...
**检查点 3**：确认 `added > 0`
- 日志应显示 `[Planner] Persisting plan with X new chapter tasks`
- Railway logs 应显示章节任务被持久化
...
- 只修改一行，影响范围小
- 利用现有函数，逻辑清晰
- 如果 planner 描述很好，会被覆盖（但仍可添加保留逻辑）

**影响范围**：
- 仅影响 novel mode 的章节生成
- 不影响 t1-t4 的执行
- 不影响非 novel mode 的流程

**回滚方案**：
- 如果出现问题，只需恢复第 1317 行即可
...
1. **修改 `run_flow.py:1317-1319`** - 核心修复
   ```python
   # 修改前：
   desc = sub.description or _chapter_description(title)
   ...
   desc = _chapter_description(sub.description or title)
   ```
   - 确保所有章节描述都通过 `_chapter_description` 处理
   - 自动添加 `CHAPTER_FULL_CONTENT` 标记
   - 通过 `_apply_planner_result_to_state` 的严格验证

2. **添加检查点 1（行 1333-1337）** - 验证章节生成
   ```python
...
**预期效果**：
- ✅ 所有章节都包含 `CHAPTER_FULL_CONTENT` 标记
- ✅ `_is_valid_chapter_candidate` 验证通过
- ✅ `added > 0`，章节成功添加到 plan
- ✅ 日志显示 `[Planner] Persisting plan with X new chapter tasks`

**新增日志标识**：
- `[Planner] Chapter {subtask_id}: has_marker={bool}` - 检查点 1
- `[Planner] Validating chapter {sub_id}: is_valid={bool}` - 检查点 2

phase 7 已完成

---

Phase 8. **扩展章节标题验证以支持中文（P0 关键修复）** ⚠️

**问题发现**（最新日志 12.11 4:09am）：
虽然 Phase 7 确保了`CHAPTER_FULL_CONTENT`标记被添加，但日志显示：
```
[Planner] After sanitization: 5 valid chapters
[Planner] No chapter tasks were added (skipping plan persistence)  ← ❌ 章节仍然被拒绝！
```

**根本原因分析**：

Phase 7解决了`has_marker`问题，但**标题验证仍然失败**：

1. **Planner生成的章节标题是中文**
   - 日志显示：`第一章:沉默的日常`, `第二章:觉醒碎片` 等
   - 这些是有效的章节标题，但被现有验证拒绝

2. **`_chapter_title_has_index`只接受英文格式** (run_flow.py:77-80)
   ```python
   def _chapter_title_has_index(title: str | None) -> bool:
       if not title:
           return False
       return bool(re.search(r"chapter\s*\d+", title, re.IGNORECASE))
       # ❌ 只匹配 "chapter 1", "Chapter2" 等英文格式
       # ❌ 拒绝 "第一章", "第1章" 等中文格式
   ```

3. **验证失败导致章节被跳过**
   - `_is_valid_chapter_candidate`返回`False`
   - `_apply_planner_result_to_state`拒绝这些章节
   - `added = 0`，没有章节被添加到plan

**修复方案** 🎯 **（待实施）**：

### **方案 A：扩展`_chapter_title_has_index`支持中文章节标识**（推荐）

**修改位置**：`run_flow.py` 第 77-80 行
...
def _chapter_title_has_index(title: str | None) -> bool:
    """
    Validate chapter title format:
    - English: "chapter 1", "Chapter2", "CHAPTER 10" etc.
    - Chinese: "第一章", "第1章", "第10章" etc.
    """
    if not title:
        return False
    ...
    # Chinese format: "第" + number + "章"
    # Supports: "第1章", "第一章", "第10章" etc.
    if re.search(r"第\s*\d+\s*章", title):
        return True

    # Chinese written numbers (optional, for completeness)
    # "第一章", "第二章", "第三章" etc.
    if re.search(r"第\s*[一二三四五六七八九十百千]+\s*章", title):
        return True
...
**原理**：
- 保留原有的英文验证（向后兼容）
- 添加中文数字验证：`第\d+章`（如"第1章"、"第10章"）
- 添加中文文字验证：`第[一二三...]章`（如"第一章"、"第二章"）
- 三种格式任意一种匹配即通过
...
**缺点**：
- ⚠️ 中文章节标题在后续流程中可能不会被标准化为"Chapter N"格式
- ⚠️ 需要确保前端UI能正确显示中文标题
...
**修改位置**：`run_flow.py` 第 1308-1327 行（Phase 6中的sanitization循环）

**修改内容**：
```python
# 在 sanitized_subtasks 构造时，添加标题标准化
for sub in parsed_subtasks:
    title = sub.get("title", "")

    # 标准化中文章节标题为英文格式
    # "第1章:XXX" → "Chapter 1: XXX"
    # "第一章:XXX" → "Chapter 1: XXX"
    title = _normalize_chinese_chapter_title(title)

    # 验证章节格式
    is_chapter = _chapter_title_has_index(title) or ...

    if not is_chapter:
        continue

    # 使用标准化后的标题
    desc = _chapter_description(sub.get("description") or title)
    sanitized_subtasks.append({
        "subtask_id": sub_id,
        "title": title,  # 使用标准化后的英文标题
        "description": desc,
        ...
    })

# 新增辅助函数
def _normalize_chinese_chapter_title(title: str) -> str:
    """Convert Chinese chapter titles to English format"""
    if not title:
        return title

    # "第1章" → "Chapter 1"
    match = re.search(r"第\s*(\d+)\s*章[：:]?\s*(.*)", title)
    if match:
        num, rest = match.groups()
        return f"Chapter {num}: {rest}".strip(": ")

    # "第一章" → "Chapter 1" (convert Chinese numerals)
    # 需要中文数字转换表
...
**原理**：
- 在验证前将中文章节标题转换为英文格式
- 保持`_chapter_title_has_index`不变
- 确保所有章节标题统一为"Chapter N"格式
...
1. **修改`_chapter_title_has_index`函数** (run_flow.py:77-80)
   ```python
   def _chapter_title_has_index(title: str | None) -> bool:
       if not title:
           return False
...
2. **添加检查点日志验证** (run_flow.py:918行前)
   ```python
   # 在 _apply_planner_result_to_state 中
   if novel_mode and idx > 4:
       title = raw.get('title', '')
       is_valid = _is_valid_chapter_candidate(raw)
       has_index = _chapter_title_has_index(title)
       has_marker = CHAPTER_FULL_CONTENT in (raw.get('description') or '')
       print(f"[Planner] Validating {sub_id}: is_valid={is_valid}, has_index={has_index}, has_marker={has_marker}, title={title[:50]}")
   ```

3. **本地测试验证**
   - 运行novel mode测试
   - 确认日志显示`has_index=True`（中文标题被接受）
   - 确认`added > 0`（章节成功添加）
...
```python
# 预期日志输出：
[Planner] Validating t5: is_valid=True, has_index=True, has_marker=True, title=第一章:沉默的日常
[Planner] Validating t6: is_valid=True, has_index=True, has_marker=True, title=第二章:觉醒碎片
```
...
# 预期日志输出：
[Planner] After sanitization: 5 valid chapters
[Planner] Persisting plan with 5 new chapter tasks  ← ✅ added > 0
[Planner] ✓ Plan and state persisted successfully
```

**检查点 3**：确认Railway logs显示章节任务被派发
- 日志应包含`[Worker] Starting subtask t5`
- 日志应包含`SUBTASK_RESULT for t5`
...
- 不影响t1-t4的执行
- 不影响非novel mode的流程
- **可能影响前端显示**：如果前端假设标题是英文，可能需要适配

**回滚方案**：
- 如果出现问题，只需恢复`_chapter_title_has_index`函数即可

---

### **后续考虑（Phase 9可选）**

如果希望前端始终显示统一格式，可以在Phase 9实施：

1. **在`_normalize_chapter_subtask`中标准化标题**
   - 将中文标题转换为"Chapter N: 中文副标题"
   - 保持描述不变

2. **或者在前端添加多语言支持**
   - 根据用户语言偏好显示"Chapter 1"或"第一章"
   - 后端保留原始标题
...
**修改文件**：`multi_agent_platform/run_flow.py`

**修改位置 1**：第 77-94 行 - 扩展标题验证 ✅
```python
def _chapter_title_has_index(title: str | None) -> bool:
    """
...
✓ "第一章:沉默的日常" -> True  (带副标题)
✓ "第二章:觉醒碎片" -> True    (带副标题)
✗ "Some random title" -> False (非章节标题)
```

**预期效果**：
- ✅ 中文章节标题（如"第一章:沉默的日常"）通过验证
- ✅ 英文章节标题（如"Chapter 1"）继续通过验证
- ✅ `_is_valid_chapter_candidate`返回`True`
- ✅ 章节任务被添加到plan（`added > 0`）
- ✅ 日志显示详细的验证过程

**新增日志标识**：
- `[Planner] Validating {sub_id}: is_valid={bool}, has_index={bool}, has_marker={bool}, title=...`
- `[Planner] ❌ Rejected {sub_id}: title does not match chapter pattern (English 'chapter N' or Chinese '第N章')`
- `[Planner] ❌ Rejected {sub_id}: description missing CHAPTER_FULL_CONTENT marker`

---

**总结**：

Phase 8通过扩展`_chapter_title_has_index`函数，使其同时接受：
1. 英文格式：`chapter\s*\d+`
2. 中文格式：`第\s*(\d+|[一二三...])\s*章`

这是**最小改动**，解决了当前planner生成中文章节标题被拒绝的问题，确保章节任务能成功添加到plan并被派发执行。

**Phase 8 已完成** ✅ (2025-12-11)
